...
// Source: sanity/queries/categoties.ts
...
// Query: *[    _type == "order" && customer->clerkUserId == $clerkUserId] | order(createdAt desc) {    _id,    orderNumber,    total,    status,    createdAt,    "itemCount": count(items),    "itemNames": items[].product->name,    "itemImages": items[].product->images[0].asset->url}
...
// Query: *[    _type == "order" && _id == $orderId][0] {    _id,    orderNumber,    clerkUserId,    email,    items[] {        _key,        quantity,        priceAtPurchase,        product-> {            _id,            name,            "slug": slug.current,            "image": images[0]{                asset->{                    _id,                    url                }            }    }},    total,    status,    address{        name,        line1,        line2,        city,        postalCode,        country,    },    stripePaymentId,    createdAt}
...
// Query: *[    _type == "order" && stripePaymentId == $stripePaymentId][0] { id }
